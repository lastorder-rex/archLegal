...
export async function GET(request: NextRequest) {
  try {
    ...
    if (consultationsError) {
      console.error('[payments/stages] failed to load consultations', consultationsError);
    ...
    // 상담글별로 결제 단계 그룹화
    const stagesByConsultation = new Map<string, Map<string, any>>();
    (allUserStages ?? []).forEach(stage => {
      if (!stagesByConsultation.has(stage.consultation_id)) {
        stagesByConsultation.set(stage.consultation_id, new Map());
      }
      stagesByConsultation.get(stage.consultation_id)!.set(stage.stage_template_id, stage);
    });

    // 각 상담글에 대해 결제 단계 매핑
    const consultationsWithStages = consultations.map(consultation => {
      const stageMap = stagesByConsultation.get(consultation.id) ?? new Map();
      ...
      return {
        consultation: {
          ...
          addressDetail: consultation.address_detail,
