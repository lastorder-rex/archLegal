...
export default async function MyPage() {
  ...
  const { data: consultationsRows } = await supabase
    .from('consultations')
    .select('id, created_at, address, address_detail, message, main_purps, tot_area, plat_area, ground_floor_cnt, phone, email, attachments')
    .eq('user_id', session.user.id)
    .eq('is_del', 'N')
    .order('created_at', { ascending: false });

  const consultations = consultationsRows ?? [];

  return (
    <main className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-8">
        <MyPageContent profile={profile} fallbackEmail={session.user.email ?? null} consultations={consultations} />
      </div>
    </main>
  );
}
